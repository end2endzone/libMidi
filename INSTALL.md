...
1) Download the source code from an existing [tags](http://github.com/end2endzone/libMidi/tags) and extract the content to a local directory (for example `c:\projects\libMidi` or `~/dev/libMidi`).
...
These are the base requirements to build and use libMidi:
...
1) Get a copy of the source code by using one of the following methods:
   * Download the source code of the project from an existing [tags](http://github.com/end2endzone/libMidi) and extract the downloaded zip file to a local directory (for example `c:\projects\libMidi` or `~/dev/libMidi`).
   * Clone the github repository by running `git clone "http://github.com/end2endzone/libMidi"`.
...
| LIBMIDI_BUILD_TEST    | BOOL   |           OFF           | Enable/disable the generation of unit tests target.        |
| LIBMIDI_BUILD_DOC     | BOOL   |           OFF           | Enable/disable the generation of API documentation target. |
| LIBMIDI_BUILD_SAMPLES | BOOL   | OFF                     | Enable/disable the generation of samples target.           |
...
# Testing #
libMidi comes with unit tests which help maintaining the product stability and level of quality.
...
To run tests, navigate to the `build/bin` folder and run `libmidi_unittest` executable. For Windows users, the executable is located in `build\bin\Release`.

Test results are saved in junit format in file `libmidi_unittest.x86.debug.xml` or `libmidi_unittest.x86.release.xml` depending on the selected configuration.
